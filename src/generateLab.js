...
const { normalizeLab } = require("./normalizeLab");
const { pickScenario } = require("./scenario-matrix/loadScenarioMatrix");
...
const vulnerabilityType = request.attackType || request.vulnerability || "xss";
const difficulty = request.difficulty || "beginner";
const scenarioId = request.scenarioId;

const selectedScenario = pickScenario(vulnerabilityType, difficulty, scenarioId);

request.scenario = selectedScenario;

console.log("[SCENARIO] Selected:", selectedScenario.id, "-", selectedScenario.title);
...
const prompt = buildLabPrompt(request);
  ...
  parsed = normalizeLab(parsed);

  const outputDir = path.resolve("generated");
  fs.mkdirSync(outputDir, { recursive: true });

  const safeLabId = parsed.labId || "generated-lab";
  const outputPath = path.join(outputDir, `${safeLabId}.json`);
